...
IngameState::IngameState() : GameState(), m_transmissionRate(0), m_currentCorrectSymbol{symbol::none}, m_distribution(0, 3) {
	...
	Application::get().setSignalLenght(6);
}
...
void IngameState::update(double deltaTime) {
	...
	if (Application::get().getGameState() == "g:r")
		return;

	if (Application::get().getGameState() == "g:o") {
		m_gameObjects["symbol"]->getSprite().setPosition(-100, -100);
		m_transmissionRate = 100;
	...
	if (TimeUtils::Physics::shouldUpdatePhysics2()) {
